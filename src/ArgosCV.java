...
public class ArgosCV {
	...
	void run() {
		...
		CascadeClassifier eye_cascade =   new  CascadeClassifier("haarcascade_eye.xml");//"haarcascade_eye.xml");
		...
		while (true) {
		    if (camera.read(frame)) {
		        ...
		        //try {
			        ...
			        eye_cascade.detectMultiScale(gray, detections, 1.1 /*scaleFactor*/, 3 /*minNeighbors*/, 1, 
			        							 new Size(30,30) /*minSize*/, new Size());
              			        // detectMultiScale(Mat, MatOfRect, double scaleFactor, int minNeighbors, int flags, Size minSize, Size maxSize)
			       // mouth_cascade.detectMultiScale(gray, eyedetections,            1.1,                1,         1, new Size(30,30), new Size());
			            
			            // Detected features
			        	Rect[] eyesArray = detections.toArray();
			            ...
			            for (int j = 0; j < eyesArray.length; j++){
			                ...
			                int radius = (int) Math.round( eyesArray[j].width/2);
...
*/
			        ...
			        //return image;
				        ...
				        //Imgcodecs.imwrite("enhancedParrot.jpg", gray);
